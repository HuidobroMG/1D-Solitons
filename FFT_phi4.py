...
Description:
    ...
    in the 1-dimensional phi4 theory.
...
# phi4 model: V = (1-phi**2)**2/2
phiK = np.tanh(x) # Kink
phiAK = -np.tanh(x) # Antikink

#------------------------------------------------------------------------------

# We could solve the equation of motion using a loop
'''
...
def EL(phi, phi_old):
    ...
    for i in range(1, Nx-1):
        phi_new[i] = (2*(1-ratio**2)*phi[i] - phi_old[i] + 
                      ratio**2*(phi[i+1]+phi[i-1]) - dt**2*dVpot[i])
...
# Create the new field from the older steps
def EL(phi, phi_old):
    dVpot = -2*phi*(1-phi**2)
...
if Animer == 1:
    ...
    anim.save('FFT_KAK_phi4.gif', writer = 'pillow')